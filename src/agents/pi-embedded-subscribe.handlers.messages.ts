...
// The duplicate-output bug (apparently an Anthropic-side issue) hasn't recurred
// in ages, so the forensic logging/dumping is OFF by default. The dedup safety
// net below stays ALWAYS on. Re-enable the diagnostics with DUP_FORENSIC_DEBUG=1.
const DUP_FORENSIC = process.env.DUP_FORENSIC_DEBUG === "1";
...
export function handleMessageEnd(
  ...
  if (DUP_FORENSIC && detectInternalRepetition(rawPreProcess)) {
    const sessionId = (ctx.params.session as { id?: string }).id ?? "unknown";
    console.log(
  ...
  if (DUP_FORENSIC && detectInternalRepetition(rawText)) {
